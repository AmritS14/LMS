...
struct LOGlassCardModifier: ViewModifier {
...
extension View {
    func loGlassCard(cornerRadius: CGFloat = 16, shadowOpacity: CGFloat = 0.08) -> some View {
        modifier(LOGlassCardModifier(cornerRadius: cornerRadius, opacity: shadowOpacity))
    }
}

// MARK: - Premium Card
struct LOPremiumCard<Content: View>: View {
    let content: Content
...
// MARK: - Gradient Card
struct LOGradientCard<Content: View>: View {
    let gradient: [Color]
    let content: Content
    var cornerRadius: CGFloat = 20
...
struct LOCircularProgress: View {
    let progress: Double
    let color: Color
...
struct LOFloatingActionButton: View {
    let icon: String
    let title: String
    let color: Color
    let action: () -> Void
...
// MARK: - Shimmer Effect
struct LOShimmerModifier: ViewModifier {
